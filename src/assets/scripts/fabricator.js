(function () {
	...
	fabricator.allItemsToggles = function () {
		...
		function toggleAllItems(type, value) {
			...
			for (var i = 0; i < _items.length; i++) {
				value ? removeClass(_items[i], 'f-item-hidden') : addClass(_items[i], 'f-item-hidden');
			}
	
			var button = document.querySelector('.f-menu [data-f-toggle-control=' + type + ']');
			value ? addClass(button, 'f-active') : removeClass(button, 'f-active');
	...
	fabricator.singleItemToggle = function () {
		...
		function toggleSingleItemCode(e) {
			...
			(button.className.indexOf('f-item-hidden') > -1) 
				? removeClass(button, 'f-item-hidden') : addClass(button, 'f-item-hidden');
		}
	};
